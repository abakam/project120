...
namespace CashvaultCore.Validation
{
    public class ValidateDataAnnotationsBehavior: Attribute, IServiceBehavior
    {
        ...
        {
            var operations = from dispatcher in serviceHostBase.ChannelDispatchers.Cast<ChannelDispatcher>()
                from endpoint in dispatcher.Endpoints
                from operation in endpoint.DispatchRuntime.Operations
                select operation;

            operations.ToList()
                .ForEach(operation=> operation.ParameterInspectors.Add(_validatingParameterInspector));
        }
    }
}
